---
title: "The Principles for More Reliable, Safer and Faster AI Agents"
...
AI agents often interact with real-world systems and even cause side-effects in real life, e.g. support bot issuing refunds. Depending on the nature of the agent, you might want to implement checkpoints for:
...
There's an excellent guide on this by [Eugene Yan - Evaluating the Effectiveness of LLM-Evaluators](https://eugeneyan.com/writing/llm-evaluators/#aligning-llm-evaluators-to-our-criteria)
...
Why? Each tool is lengthy JSONSchema that is passed as a system prompt to the LLM. It costs money, it hurts the performance and can be hard to tune. Modular approach simplifies the system, like the planner-executor division. It also makes it easier to tune each agent independently without touching the rest of the system.

### 6. Implement Strong Guardrails

Use robust validation tools at every step:

- Strict input/output validation, especially for tool calls: Python with [Instructor](https://python.useinstructor.com/) or Pydantic, TypeScript with Zod (with is built-in into brilliant `ai` library). These guardrails prevent agents from straying outside their intended parameters and ensure reliable operation.
...
RAG is a technique that allows agents to retrieve information from a database or external sources.
