...
Why? Each tool is lengthy `JSONSchema` that is passed as a system prompt to the LLM. It costs money, it hurts the performance and can be hard to tune. Modular approach simplifies the system, like in the planner-executor division each of the subagents have an independent, task-specific prompt which makes it easier to tune each piece independently without touching the rest of the system.
...
[RAG](/posts/make-your-rag-suck-less/) is a technique that allows agents to retrieve information from a database or external sources. Without your domain knowledge, your agent is going to be generic, with the access to the same information as any other agent. You data gives you the edge.
